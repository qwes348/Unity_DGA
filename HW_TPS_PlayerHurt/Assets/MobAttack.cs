...
public class MobAttack : StateMachineBehaviour, IHitBoxResponder
{
    public int damage = 5;
    ...
    public void CollisionWith(Collider collider, HitBox hitbox)
    {
        ...
        hitBox.GetContactInfo(from: hitbox.gameObject.transform.root.transform.position,
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if(normalizedTime >= 1f)
            normalizedTime = normalizedTime - Mathf.Floor(normalizedTime);  // loop를돌때 normalizedTime이 1보다 커지는걸 방지

        if (0.5f <= normalizedTime && normalizedTime <= 0.6f)  // 타격타이밍 맞추기
            hitBox.UpdateHitBox();
        if (normalizedTime >= 0.8f)
            hitBox.hitObjects.Clear();        
    }
