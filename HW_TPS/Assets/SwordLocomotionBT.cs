...
public class SwordLocomotionBT : StateMachineBehaviour
{
    ...
    {
        pc.FrameMove();
        if(Input.GetKeyDown(KeyCode.Alpha1) && !animator.IsInTransition(0))
        {
            animator.SetTrigger("DisarmSword");
        }
        if(Input.GetMouseButtonDown(0) && !animator.IsInTransition(0))
        {
            animator.SetTrigger("OnSwordAttack1");
        }
