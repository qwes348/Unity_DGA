...
public class StatusController : MonoBehaviour
{
    ...
    // 목마름이 줄어드는 속도
    [SerializeField]
    private int thirstyDecraseTime;
    ...
    private void Hungry()
    {
        if(currentHungry > 0)
        {
            if(currentHungryDecreaseTime <= hungryDecraseTime)
    ...
    private void Thirsty()
     {
        if(currentThirsty > 0)
        {
            if(currentThirstyDecreaseTime <= thirstyDecraseTime)
