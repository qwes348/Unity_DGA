Shader "Custom/CustomPhong"
{
    ...
    SubShader
    {
        ...
        #pragma surface surf CustomPhong noambient
		...
		float4 LightingCustomPhong(SurfaceOutput s, float3 lightDir, float3 viewDir, float atten)
		{
			float4 final;
			...
			//float3 R = 2 * s.Normal * (dot(s.Normal, viewDir)) - viewDir;  // reflect함수 안쓰고 R벡터 구하기
			float spec = saturate(dot(R, viewDir));
			//spec = pow(spec, 100);
			spec = pow(spec, _SpecPow);
			specularColor = spec * _SpecCol.rgb;
