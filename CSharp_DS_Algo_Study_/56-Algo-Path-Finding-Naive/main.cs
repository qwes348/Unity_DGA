...
public class NaiveSerach
{
  ...
  public IEnumerable<Node> Neighbors(Node c)
  {
    ...
    if(x > 0 && costGrid[x-1][y] == 0 && grid[x-1][y] != 1)
      yield return new Node(x-1, y);  // 중복된 이웃도 새로만드는 문제가 있음
    if(y < grid[0].Length-1 && costGrid[x][y+1] == 0 && grid[x][y+1] != 1)  
      yield return new Node(x, y+1);  // 
    if(x < grid.Length-1 && costGrid[x+1][y] == 0 && grid[x+1][y] != 1)      
      yield return new Node(x+1, y);
    if(y > 0 && costGrid[x][y-1] == 0 && grid[x][y-1] != 1) 
      yield return new Node(x, y-1);
  }
}
